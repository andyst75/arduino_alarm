/*
 * https://github.com/adafruit/Adafruit-ST7735-Library
 * https://github.com/adafruit/Adafruit-GFX-Library
 *
 * В качестве основы для работы с русскими буквами использована идея и исходный код "arduinec" взятый по адресу  
 * http://arduino.ru/forum/programmirovanie/rusifikatsiya-biblioteki-adafruit-gfx-i-vyvod-russkikh-bukv-na-displei-v-kodi
*/
#include <avr/pgmspace.h> 

#ifndef MyTFT_H
#define MyTFT_H

#include <TFT.h>

class MyTFT : public TFT {
public:
  MyTFT(uint8_t CS, uint8_t RS, uint8_t RST);

  void  // выводим символ 'c' в точку (x,y) с заданным цветом 'color' и размером 'size'.
        // если цвет фона 'bg' не совпадает с цветом символа, то фон закрашивается в заданный цвет
        drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size),
        
        // старый символ 'c1' в позиции (х,у) закрашиваем цветом фона 'bg', и на это место рисуем новый символ 'c2' размером 'size'
        // и цветом 'color'
        reDrawChar(int16_t x, int16_t y, unsigned char c1,unsigned char c2, uint16_t color, uint16_t bg, uint8_t size),
        
        // выводим строку в заданные координаты, если задана предыдущая строка, то выводим только несовпадающие символы
        drawText(int16_t x, int16_t y, char *s, char *s_prev, uint16_t color, uint16_t bg, uint8_t size),
        
        // выводим строку UTF8 в заданные координаты, если задана предыдущая строка, то выводим только несовпадающие символы
        drawUTF8Text(int16_t x, int16_t y, char *s, char *s_prev, uint16_t color, uint16_t bg, uint8_t size),
        
        // перекодируем строку из UTF8 в cp1251
        utf8rus(char *source, char *dest); 
};

#endif // MyTFT_H

#ifndef FONT5X7_H
#define FONT5X7_H

// standard ascii+cp1251 5x7 font

static const unsigned char  font[] PROGMEM = {

};
#endif //FONT5X7_H

